        ...
        <div class="category-card mb-3">
            <p class="mb-0 category-header">
                <a class="jobSubItem btn btn-link w-100 text-start d-flex align-items-center" data-toggle="collapse"
                    href="#collapse<?= $subJob['sub_job_category']['idtbl_sub_job_category'] ?>" role="button"
                    aria-expanded="false"
                                    ...
                                    <div class="col-md-6">
                                        <div class="option-select-container">
                                            ...
                                            <select class="form-select job-option-select job_option_f"
                                                ...
                                                data-option-id="<?= $optionId ?>" data-option-group="<?= $groupId ?>"
                                        ...
                                        <div class="pricing-section bg-light p-2 rounded">
                                            <div class="row g-2">
                                                <div class="col-md-4">
                                                    ...
                                                    <input
                                                        class="form-control form-control-sm text-end item-price item_price_f"
                                                        type="number" step="any" id="item_price_<?= $uniqueKey ?>"
                                                        name="item_price_<?= $uniqueKey ?>"
                                                        data-original_price="<?= $price ?>"
                                                        data-uniq-id="<?= $uniqueKey ?>" value="<?= $list_price ?>"
                                                        onkeyup="schedulePriceUpdate(this);">
                                                </div>
                                                <div class="col-md-3">
                                                    <label class="form-label small mb-1">Qty</label>
                                                    <input
                                                        class="form-control form-control-sm text-end item-qty item_qty_f"
                                                        type="number" step="any" id="item_qty_<?= $uniqueKey ?>"
                                                        name="item_qty_<?= $uniqueKey ?>"
                                                        data-uniq-id="<?= $uniqueKey ?>" value="<?= $qty ?>"
                                                        onkeyup="schedulePriceUpdate(this);">
                                                </div>
                                                <div class="col-md-5">
                                                    <label class="form-label small mb-1">Total</label>
                                                    <input
                                                        class="form-control form-control-sm text-end item-total-price item_total_price_f"
                                                        type="number" step="any" readonly
                                                        id="item_total_price_<?= $uniqueKey ?>"
                                                        name="item_total_price_<?= $uniqueKey ?>"
                                                        data-uniq-id="<?= $uniqueKey ?>" value="<?= $total ?>">
                                                </div>
                                            </div>
                                            ...
                                            <div class="row g-2 mt-2">
                                                <div class="col-md-4">
                                                    ...
                                                    <select class="form-select form-select-sm line_discount_type"
                                                        ...
                                                        <option <?= ($line_discount_type == '1') ? 'selected' : '' ?>
                                                            value="1">%</option>
                                                        <option <?= ($line_discount_type == '2') ? 'selected' : '' ?>
                                                            value="2">Amount</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-3">
                                                ...
                                                <div class="col-md-5">
                                                    ...
                                                    <input class="form-control form-control-sm text-end item-net-price"
                                                        ...
                                                        data-uniq-id="<?= $uniqueKey ?>" value="<?= $net_amount ?>">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row g-2 mt-2">
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control form-control-sm"
                                                id="remark_<?= $uniqueKey ?>" name="remark_<?= $uniqueKey ?>"
                                                value="<?= isset($option['remark']) ? htmlspecialchars($option['remark']) : '' ?>"
    ...
    jobData.data.forEach(item => {
        ...
        item.job_options.forEach(jobOptionObj => {
            ...
            jobOptions.forEach(option => {
                ...
                if (jobOption.OptionType === "Conditional" && jobOption
                    .job_details_option_value && Array.isArray(option.option_values)) {
    ...
    const xhr = $.ajax({
        ...
        success: function(res) {
            if (!res.status || !res.data || res.data.length === 0) {
                $(childWrapperSelector).html('');
                Promise.resolve(
                    getOptionValuePrice(selectElement, subJobCategoryID, optionGroupID,
                        selectedOptionValue, jobOptionID, $selectedOption, insertOption, parentUniqueKey
                    )
            ...
            let html = res.data.map(group => {
                ...
                let uniqueKey =
                    `${escapeHtml(jobOption.JobSubcategoryID)}_${escapeHtml(jobOption.OptionGroupID)}_${escapeHtml(jobOption.JobOptionID)}`;
            ...
            Promise.all(childOptionPromises)
                .then(() => {
                    return Promise.resolve(
                        getOptionValuePrice(selectElement, subJobCategoryID, optionGroupID,
                            selectedOptionValue, jobOptionID, $selectedOption, insertOption,
                            parentUniqueKey)
                    );
                })
                .finally(completeAjaxCall);
...
function getOptionValuePrice(selectElement, subJobCategoryID, optionGroupID, selectedOptionValue, jobOptionID,
    $selectedOption, insertOption, parentUniqueKey) {
    if (insertOption == '2') {
        return Promise.resolve();
    }
    ...
    const xhr = $.ajax({
        ...
        success: function(result) {
            if (result.status && selectedOptionValue === lastRequestedOptionValueId) {
                const linePriceSelector = '#unit_price_' + subJobCategoryID + '_' + optionGroupID + '_' +
                    jobOptionID;
