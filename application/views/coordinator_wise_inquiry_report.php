                ...
                <div class="container-fluid">
                    <div class="page-header-content py-3">
                        <h1 class="page-header-title">
                            ...
                            <span>Coordinator wise inquiry Report</span>
                        </h1>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-12">
                            <form id="filter_table">
                                <div class="col-12">
                                    <div class="form-row">
                                        <div class="col-2">
                                            <label class="small font-weight-bold text-dark">Coordinator*</label>
                                            <select class="form-control form-control-sm selecter2 px-0"
                                                name="f_coordinator" id="f_coordinator">
                                                <option value="">Select</option>
                                                <?php foreach ($coordinatorlist->result() as $rowcoordinatorlist) { ?>
                                                <option value="<?php echo $rowcoordinatorlist->idtbl_coordinator ?>">
                                                    <?php echo $rowcoordinatorlist->coordinator_name ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>

                                        <div class="col-2">
                                            <label class="small font-weight-bold text-dark">Third Follow up*</label>
                                            <select class="form-control form-control-sm selecter2 px-0"
                                                name="f_third_follow" id="f_third_follow">
                                                <option value="">Select</option>
                                                <option value="1">Third Follow up</option>
                                                <option value="0">Non-Third Follow up</option>
                                            </select>
                                        </div>

                                        <div class="col-2">
                                            <label class="small font-weight-bold text-dark">Appointment Status*</label>
                                            <select class="form-control form-control-sm selecter2 px-0"
                                                name="f_appointment" id="f_appointment">
                                                <option value="">Select</option>
                                                <option value="1">Confirm Appointment</option>
                                                <option value="0">Not Confirm Appointment</option>
                                            </select>
                                        </div>
...
$(document).ready(function() {
    ...
    $("#f_coordinator").select2({
        placeholder: "Select Inquiry Source",
        allowClear: true
    });
    $("#f_third_follow").select2({
        placeholder: "Select Sales Person",
        allowClear: true
    });
    $("#f_appointment").select2({
        placeholder: "Select Sales Person",
        allowClear: true
    });
    ...
    $("#filter_table").submit(function(event) {
        ...
        var coordinator = $("#f_coordinator").val();
        var third_follow = $("#f_third_follow").val();
        var appointment = $("#f_appointment").val();
        var date_from = $("#date_from").val();
        var date_to = $("#date_to").val();
        
        $("#search_button").html('<i class="fas fa-spinner fa-pulse"></i>')
        setTimeout(function() {
            filterData(coordinator, third_follow, appointment, date_from, date_to);
            }, 2000);
       
    });
});

function filterData(coordinator, third_follow, appointment, date_from, date_to) {
    ...
    $('#dataTable').DataTable({
        ...
        ajax: {
            url: "<?php echo base_url() ?>scripts/coordinatorwisereportlist.php",
            type: "POST", // you can use GET
            "data": function(d) {
                return $.extend({}, d, {
                    "company_branch_id": '<?php echo ($_SESSION['branch_id']); ?>',
                    "search_coordinator": coordinator,
                    "search_third_follow": third_follow,
...
function exportPDF() {
    ...
    const bodyData = $('#dataTable').DataTable().rows({ filter: 'applied' }).data().toArray().map(row => {
        return selectedColumns.map(colIdx => {
            // If column is appointment date, check appointment status
            if (columnMapping[colIdx] === 'appointmentdate') {
                return row['appointment'] == 1 ? row['appointmentdate'] : ''; // Show date only if appointment is confirmed
            }
            return row[columnMapping[colIdx]];
        });
    });
    ...
    doc.autoTable({
        ...
        didDrawPage: function(data) {
            ...
            doc.text('Coordinator wise inquiry Report Information', doc.internal.pageSize.getWidth() / 2,
                30, {
                    align: 'center'
                });
            doc.setFont("Roboto", "normal");
    ...
    doc.save('Coordinator wise inquiry Report Information.pdf');
}
...
function exportExcel() {
    ...
    XLSX.writeFile(wb, 'Coordinator_wise_inquiry_Report_Information.xlsx');
}
